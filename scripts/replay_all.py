...
def run_replay(source_dir: str, target_dir: str):
    ...
    # Launch the ReplayLatticeGenerator from its assembled fat jar
    source_abs = os.path.abspath(source_dir)
    target_abs = os.path.abspath(target_dir)
    jar = utils.path_to_jar("replay-lattice-generator.jar")
    cmd = f"java -jar {jar} {source_abs} {target_abs}"
    try:
...
def main():
    ...
    # We use ThreadPoolExecutor to run multiple replay processes in parallel if desired,
